...
def success_pih(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    error_xyz = torch.sum(torch.abs(distance_xyz), dim=-1)
    error_ang = torch.sum(torch.abs(distance_ang), dim=-1)
    # print("error_xyz", error_xyz)
    ...
    success_ang = torch.where(
        error_ang < 8e-2,
        torch.ones_like(error_ang),
        torch.zeros_like(error_ang),
        )
    
    # print("error_xyz", success_xyz)
    # print("error_ang", success_ang)
    
    is_successed = success_xyz * success_ang

    return torch.logical_and(success_xyz, success_ang)
