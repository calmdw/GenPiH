...
from isaaclab.utils.math import combine_frame_transforms, subtract_frame_transforms, quat_error_magnitude, quat_mul, compute_pose_error
...
def orientation_command_error_tanh(env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    distance = torch.norm(curr_quat_w - des_quat_w, dim=1)
    return 1 - torch.tanh(distance / std)

def object_ee_distance(
    env: ManagerBasedRLEnv,
    std: float,
    object_cfg: SceneEntityCfg = SceneEntityCfg("base"),
...
###################################### Fluently_peg_in_hole sepecific ######################################
...
def orientation_command_error(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    # print("des_quat_b", des_quat_b)
    # print("des_quat_w", des_quat_w)
    curr_quat_w = asset.data.body_state_w[:, asset_cfg.body_ids[0], 3:7]  # type: ignore
    # print("body_ids", asset_cfg.body_ids[0])
    # print("curr_quat_w", curr_quat_w)
    # print("  " * 10)
    distance_ang = quat_error_magnitude(curr_quat_w, des_quat_w)
    error_ang = torch.sum(torch.abs(distance_ang), dim=-1)
    return distance_ang

def ori_aligned_reward(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize tracking of the position error using L2-norm.
    ...
    xy_error = torch.sum(torch.abs(distance_xyz[:, :3]), dim=-1)
    ...
    ori_aligned_reward = 1 * is_ori_aligned
    ...
    return ori_aligned_reward

def xy_aligned_reward(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize tracking of the position error using L2-norm.
    ...
    xy_error = torch.sum(torch.abs(distance_xyz[:, :2]), dim=-1)
    ang_error = torch.sum(torch.abs(distance_ang), dim=-1)
    ...
    # print("des_pose_w", des_pose_w)
    # print("current_pose_w", curr_pose_w)

    # print("curr_pos_wrthole", curr_pos_wrthole)
    # print("curr_quat_wrthole", curr_quat_wrthole)
    # print("curr_pos_wrthole", curr_pos_wrthole)
    # print("curr_quat_wrthole", curr_quat_wrthole)

    # print("distance_xyz", distance_xyz)
    # print("distance_ang", distance_ang)

    # print("pos_error", xy_error)
    # print("ori_error", ang_error)
    ...
    is_ori_rough_aligned = torch.where(
        ang_error < 1e-1,
        torch.ones_like(ang_error),
        torch.zeros_like(ang_error),
        )
    ...
    # distance_xy = distance_xyz[:, :2]
    # error_xy = torch.sum(torch.abs(distance_xy), dim=-1)
    xy_aligned_reward = 1 * is_xy_aligned * is_ori_rough_aligned
